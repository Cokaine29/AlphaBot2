...
void parseAndExecuteCommand(char* command) {
  if (strstr(command, "\"Calibrate\":\"Start\"") != NULL) {
    ...
    PCF8574Write(0xDF & PCF8574Read()); // beep on
  }
  else if (strstr(command, "\"BZ\":\"off\"") != NULL) {
    PCF8574Write(0x20 | PCF8574Read()); // beep off
  }
}
...
void setup() {
  delay(1000);
  Serial.begin(9600); // Bluetooth UART (HM-10 default)
  Wire.begin();
  pinMode(PWMA,OUTPUT);                     
  pinMode(AIN2,OUTPUT);      
  pinMode(AIN1,OUTPUT);
...
void loop() {
  while (1)
  {
    if (!follow_segment())
    {
      ...
      // Check if we exhausted the entire maze and returned to start
      if (path_length >= 2 && path[path_length-2] == 'B' && path[path_length-1] == 'B')
      {
        SetSpeeds(0, 0);
        break; // Maze exhausted without finding an end
      }
      
      lasttime = millis(); // Reset blind time counter after slow Serial prints
      continue;
    }
    ...
    // Found large intersection indicating the end
    if (sensorValues[1] > 600 && sensorValues[2] > 600 && sensorValues[3] > 600)
    {
      SetSpeeds(0, 0);
      break;
  ...
  // Learning loop finished, check if successful or failed
  ...
  if (foundEnd) {
    ...
    // Returned to start, no end found
    while (1)
    {
      SetSpeeds(0, 0);
      Serial.println("No End Found!");
      
      // Flash Red LEDs to indicate failure
      ...
      // Reset path so we can try learning again from scratch
      path_length = 0;
      return; // Return triggers loop() to restart from the beginning
    }
  }
}
